...
#' @importFrom HMDHFDplus getHMDcountries readHMDweb readHFDweb
#' @examples
#' 
#' 
...
countries <- getHMDcountries()$CNTRY
...
df <- readHMDweb(CNTRY=country,item="fltper_1x1",username=user,password=pw_HMD,fixup=TRUE)
dm <- readHMDweb(CNTRY=country,item="mltper_1x1",username=user,password=pw_HMD,fixup=TRUE)

# ============  Read conditional age-specific fertility rates ===============
fert_rates <- readHFDweb(CNTRY=country,item="mi",username=user,password=pw_HFD,fixup=TRUE)

# ======  Extract population from HMD  ====
dpopHMD <- readHMDweb(CNTRY=country,item="Population",username=user,password=pw_HMD,fixup=TRUE) 
...
db <- readHMDweb(CNTRY=country,item="bltper_1x1",username=user,password=pw_HMD,fixup=TRUE) 
# Read fertility table
fertTable <- readHFDweb(CNTRY=country,item="pft",username=user,password=pw_HFD,fixup=TRUE)
attr(fertTable, "data") <- paste ("HFD",country,sep=" ")

# Read age-specific fertility rates
asfrVV <- readHFDweb(CNTRY=country,item="asfrVV",username=user,password=pw_HFD,fixup=TRUE)
# Read TFR (period)
tfrRR <- readHFDweb(CNTRY=country,item="tfrRR",username=user,password=pw_HFD,fixup=TRUE)

# ======  Part B Extract female population exposure by parity from HFD ====
#  Population from HFD: Exposure in conditional fertility rates 
dpopHFD <- readHFDweb(CNTRY=country,item="exposRRpa",username=user,password=pw_HFD,fixup=TRUE)
